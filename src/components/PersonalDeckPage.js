...
export default function PersonalDeckPage() {
  ...
  // Return to personal decks page
  const handleReturn = () => {
    navigate('/molecules/personal-decks');
  };
  ...
  // Load deck and initialize study data
  useEffect(() => {
      ...
      try {
        ...
        try {
          if (isGuidedMode) {
            ...
            const studyData = await getStudyCards(user.uid, deckId, deckData.cards);
            if (!studyData || !studyData.currentBatch || studyData.currentBatch.length === 0) {
              ...
              setStudyData({
                ...
                progress: {
                  ...
                  masteredCards: 0,
                  currentBatchSize: deckData.cards.length,
  ...
  const handleRating = async (cardId, newValue) => {
    ...
    try {
      ...
      await setDoc(ratingsRef, {
        ...
        [`${deckId}_lastUpdated`]: timestamp
      }, { merge: true });

      // Update learning progress and get next review time
      const { reviewInterval } = await updateCardProgress(user.uid, deckId, cardId, newValue);

      // Get updated study data
      const newStudyData = await getStudyCards(user.uid, deckId, deck.cards);
